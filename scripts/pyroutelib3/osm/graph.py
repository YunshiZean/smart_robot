...
from ..protocols import DistanceFunction
...
class Graph(SimpleGraph[GraphNode]):
    ...
    def __init__(self, profile: Profile) -> None:
        ...
        self.grapher_builder = None

    def find_nearest_node(
        self, position: Position, distance: DistanceFunction = haversine_earth_distance
    ) -> GraphNode:
        ...
        return min(
            ...
            key=lambda nd: distance(position, nd.position),
        )

    def find_nearest_edge(self, position: Position):
        min_dist = None
        node1_id = None
        node2_id = None
        for key1, value in self.edges.items():
            line_point1 = self.nodes[key1].position
            for key2 in value.keys():
                line_point2 = self.nodes[key2].position
                tmp_dis = dis_point_to_seg_line(position, line_point1, line_point2)
                # print("tmp_dis=", tmp_dis)
                if min_dist is None:
                    min_dist = tmp_dis
                    node1_id = key1
                    node2_id = key2
                elif tmp_dis < min_dist:
                    min_dist = tmp_dis
                    node1_id = key1
                    node2_id = key2
        # print("min_dist=%d %d %f" % (node1_id, node2_id, min_dist))
        # print("edge=", node1_id, self.edges[node1_id])
        return self.nodes[node1_id], self.nodes[node2_id]
    ...
    def add_features(self, features: Iterable[reader.Feature]) -> None:
        ...
        * for duplicate turn restrictions - the incoming restriction is processed,
        ...
        """
        self.grapher_builder = _GraphBuilder.add_features_to(self, features)

    @classmethod
    def from_features(
        cls, profile: Profile, features: Iterable[reader.Feature]
    ) -> Self:
        ...
        if member.type == "node" and member.role == "via":
            if member.ref not in self.g.nodes:
                raise _InvalidTurnRestriction(
                    r, f"reference to unknown node: {member.ref}"
                )
            return [member.ref]
            ...
            if not nodes:
                raise _InvalidTurnRestriction(
                    r, f"reference to unknown way: {member.ref}"
                )
            return nodes
...
class _GraphChange:
    ...
    def __init__(self, g: Graph) -> None:
        ...
        self.phantom_node_id_counter = (
            self.g._phantom_node_id_counter
        )  # pyright: ignore
    ...
    def restriction_as_cloned_nodes(self, osm_nodes: List[int]) -> Optional[List[int]]:
        ...
        for osm_node_id in osm_nodes[1:]:
            ...
            if not is_clone and not is_last:
                ...
                self.edges_to_add.setdefault(previous_node_id, {})[
                    cloned_node_id
                ] = cost
            else:
...
class _InvalidTurnRestriction(ValueError):
    ...
    def __init__(self, restriction: reader.Relation, reason: str) -> None:
        super().__init__(
            f"invalid turn restriction {restriction.id}: {reason} - skipping"
        )
